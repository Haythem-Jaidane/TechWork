...
void supprimer (ActionEvent event) throws IOException {
    ...
    if (selectedIndex != -1) {
        ...
        sp.supprimer(new Utilisateur(sp.getPostIdByName(tfcin.getText()),tfcin.getText() ,tfnom.getText() ,tfprenom.getText()  ,tfmotDePasse.getText()   , tfemail.getText()  ,   tfrole.getText()));     // update database
        JOptionPane.showMessageDialog(null, "Utilisateur supprimer !");
        
        tableView.refresh(); // update display

    } else {
        JOptionPane.showMessageDialog(null, "Sélectionnez un utilisateur à supprimer.");
    }
}
